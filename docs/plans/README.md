...
- [第二十次迭代：Go2 WebRTC 传输与 Skill / Tool 能力契约收口](./2026-07-17-210000-go2-webrtc-transport-skill-tool-contract.md)：明确 WebRTC 是底层 transport，Skill / Tool 是上层能力契约；收口 odom 映射、Go2 tool metadata、planner 可见能力和真机 acceptance trace。
- [第二十一次迭代：可替换导航 Provider 与 Fake 验证底座](./2026-07-22-150000-navigation-provider-foundation.md)：以 `NavigationClient` 隔离 DimOS/Nav2/Go2 后端，mock 自动装配 Fake，提供相对目标、状态查询和取消能力；真机仅在显式注入 Provider 后向规划器暴露。
